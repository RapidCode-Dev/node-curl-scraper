...
class CurlImpersonate {
    ...
    async request(url, options = {}, fingerprintName) {
        ...
        try {
            ...
            // Debug logging
            debug_1.debugLogger.logCurlError(error, 'request execution');
        ...
        for (const [key, value] of Object.entries(fingerprint.headers)) {
            if (value) {
                ...
                const isOverridden = options.headers &&
                    Object.keys(options.headers).some(customKey => customKey.toLowerCase() === key.toLowerCase());
                if (!isOverridden) {
                    args.push('-H', `${key}: ${value}`);
                }
            }
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                if (code === 0) {
                    ...
                    // Debug logging with file saving
                    debug_1.debugLogger.logRawCurlWithFile(args, stdout, stderr, 'curl-error', url);
                    // Create detailed error message with CURL error code information
                    const errorInfo = code !== null && code in types_1.CURL_ERROR_CODES ? types_1.CURL_ERROR_CODES[code] : null;
                    ...
                    error.curlCode = code;
                    error.curlCodeName = errorInfo?.name;
                    error.stderr = stderr;
                    error.stdout = stdout;
                    reject(error);
                }
            ...
            child.on('error', (error) => {
                // Debug logging with file saving
                debug_1.debugLogger.logRawCurlWithFile(args, '', error.message, 'curl-spawn-error', url);
                reject(error);
            });
        });
    }
     ...
     */
    parseResponse(output, originalUrl, responseTime, options = {}) {
    ...
    parseError(error) {
        ...
        const errorInfo = curlCode !== null ? types_1.CURL_ERROR_CODES[curlCode] : null;
    ...
    extractCurlErrorCode(message) {
        ...
        for (const pattern of patterns) {
            ...
            if (match) {
                ...
                if (code in types_1.CURL_ERROR_CODES) {
                    return code;
                }
            }
        }
     ...
     */
    isProxyRelatedError(message, curlCode) {
